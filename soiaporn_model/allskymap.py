...
Edited 2018 by Francesca Capel:
- patch from Tobias Winchen (2012)
- extra cuts on using the tissot method
- extra parameters in label_meridians
"""
...
class AllSkyMap(Basemap):
    ...
    * tissot, which overrides Basemap.tissot, correctly handling geodesics that
    ...
    def label_meridians(self, lons, fontsize=10, valign='bottom', vnudge=0,
                        halign='center', hnudge=0, color='white', alpha=1):
        """
        ...
        for lon,lat in zip(lons, lats):
            ...
            pl.text(x, y, angle_symbol(lon_lbl), fontsize=fontsize,
                    ...
                    horizontalalignment=halign, color=color, alpha=alpha)
    ...
    def tissot(self,lon_0,lat_0,radius_deg,npts,ax=None,**kwargs):
        ...
        # edit FC: make restriction more strict as some were slipping though
        if ((self.east_hem(lon_0) and self.east_hem(lon_0+radius_deg+5)) or (not self.east_hem(lon_0) and not
            self.east_hem(lon_0-radius_deg-2))) and abs(lat_0) + radius_deg <90:
