...
class AllSkyMap(Basemap):
    ...
    * add function for skymap label plotting given a certain style
    ...
    def draw_standard_labels(self, cmap, textcolor):
        ...
        self.drawparallels(np.arange(-75, 76, 15), linewidth = 0, dashes = [1,2],
                             labels=[1, 0, 0, 0], textcolor = textcolor, fontsize = 14);
        self.drawmeridians(np.arange(-150, 151, 30), linewidth = 0, dashes = [1,2]);
    ...
    def tissot(self,lon_0,lat_0,radius_deg,npts,ax=None,**kwargs):
        ...
        if (    (    (self.east_hem(lon_0) and self.east_hem(lon_0 + radius_deg + 5))
                     or (not self.east_hem(lon_0) and not self.east_hem(lon_0-radius_deg-2))    )
