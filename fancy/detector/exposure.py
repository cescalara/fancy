...
def m(theta, p):
    return p[0] * np.sin(theta) * np.sin(alpha_m(theta, p)) + alpha_m(theta, p) * p[
        1
    ] * np.cos(theta)
...
def integrand(phi, theta, varpi, kappa, p):
    ...
    """
    omega = [np.sin(theta) * np.cos(phi), np.sin(theta) * np.sin(phi), np.cos(theta)]
    if kappa > 100:
        integ = (
            (p[3] / p[4])
            * np.exp(
                kappa * np.dot(omega, varpi)
                + np.log(kappa)
                - np.log(4 * np.pi / 2)
                - kappa
            )
        ...
        integ = (
            (p[3] / p[4])
            * constant_val(kappa)
            * np.exp(kappa * np.dot(omega, varpi))
            * m(theta, p)
            * np.sin(theta)
        )
    return integ
...
def integrand_gmf_simple(phi, theta, varpi, kappa, kappa_gmf, p):
    """
    Integrand for \int d(omega) m(omega) * rho(omega | kappa).
    ...
    NB: the alpha_T / M factor from eps(omega) is included
    NB: the kappa / 4pi*sinh(kappa) from the vMF is included
    NB: accounts for simple GMF deflections assuming one dist
        dominates
    """

    # Approx: assume wider dist dominates (ok for p+)
    if kappa_gmf < kappa:
        kappa_in = kappa_gmf
    else:
        kappa_in = kappa
    ...
    if kappa_in > 100:
        ...
        integ = (
            (p[3] / p[4])
            * constant_val(kappa_in)
            * np.exp(kappa_in * np.dot(omega, varpi))
            * m(theta, p)
            * np.sin(theta)
        )
    return integ
...
def integrand_vMF(phi, theta, varpi, kappa):
    ...
    if kappa > 100:
        integ = np.exp(
            kappa * np.dot(omega, varpi) + np.log(kappa) - np.log(4 * np.pi / 2) - kappa
        ) * np.sin(theta)
    else:
        integ = (
            constant_val(kappa) * np.exp(kappa * np.dot(omega, varpi)) * np.sin(theta)
        )
    return integ
...
def alpha(theta, phi, varpi):
    ...
    """
    inner = (
        varpi[0] * np.sin(theta) * np.cos(phi)
...
def m_dec(dec, p):
    return p[0] * np.cos(dec) * np.sin(alpha_m_dec(dec, p)) + alpha_m_dec(dec, p) * p[
        1
    ] * np.sin(dec)
