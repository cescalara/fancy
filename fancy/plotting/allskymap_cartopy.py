...
class AllSkyMapCartopy():
    ...
    def add_grid_labels(self,
        ...
        for lon in np.arange(x0 + dx, x1, dx, dtype=dtype):
            if reverse:
                ...
                label = '{0}$\degree$'.format(lon)

            if top:
                text = self.ax.text(lon,
                                    y1,
                                    r'{0}$^\circ$\n\n'.format(lon),
                                    va='center',
                                    ha='center',
    ...
    def scatter(self, lons, lats, color, **kwargs):
        ...
        '''
        return self.ax.scatter(180. - lons,
                               lats,
                               c=color,
                               transform=ccrs.PlateCarree(),
                               **kwargs)
