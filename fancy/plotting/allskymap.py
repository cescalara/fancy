...
class AllSkyMap(Basemap):
    ...
    * tissot, which overrides Basemap.tissot, correctly handling geodesics that
    ...
    Edits April 2018 by Francesca Capel:

    * patch from Tobias Winchen 2012 
    ...
    * catch errors in pyproj inv() methodand print warning
    ...
    def drawmapboundary(
        self, color="k", linewidth=1.0, fill_color=None, zorder=None, ax=None
    ):
        """
    ...
    def draw_standard_labels(self, minimal=False, fontsize=14):
        ...
        if minimal:
            ...
            self.label_meridians(
                lons, fontsize=fontsize, vnudge=1, halign="left", hnudge=-3, color="k"
            )
            ...
            lons = np.arange(-150, 151, 60)
            self.label_meridians(
                lons, fontsize=fontsize, vnudge=1, halign="left", hnudge=-1, color="k"
            )
    ...
    def tissot(self, lon_0, lat_0, radius_deg, npts, ax=None, **kwargs):
        ...
        if (
            (self.east_hem(lon_0) and self.east_hem(lon_0 + radius_deg + 7))
            or (not self.east_hem(lon_0) and not self.east_hem(lon_0 - radius_deg - 7))
        ) and abs(lat_0) + radius_deg < 90:

            return Basemap.tissot(
                self, lon_0, lat_0, radius_deg, npts, ax=None, **kwargs
            )
        ...
        if abs(lat_0) + radius_deg >= 90:
            ...
            x, y = self(
                -180 * sign(lat_0) * ones(N), sign(lat_0) * (90 - array(r) * dL)
            )
            segs.extend(zip(x, y))
...
if __name__ == "__main__":
    ...
    map.label_meridians(
        lons, fontsize=9, vnudge=1, halign="left", hnudge=-1
    )  # hnudge < 0 shifts to right
    ...
    line = plot([rt2 + 0.1, 0 + 0.1], [rt2, 2 * rt2], "m-", clip_path=limb)
    ...
    lines = map.geodesic(120, 30, 240, 60, clip=False, c="b", lw=7, alpha=0.5)
    lines = map.geodesic(240, 60, 120, 30, c="r", lw=3, alpha=0.5)
    ...
    # brown hue.
    lines = map.geodesic(240, -60, 120, 30, c="b", lw=2, alpha=0.5)
    lines = map.geodesic(120, 30, 240, -60, c="y", lw=2, alpha=0.5)
    ...
    poly = map.tissot(-175, 30, 15, 100, color="r", alpha=0.6)
    # lower right:
